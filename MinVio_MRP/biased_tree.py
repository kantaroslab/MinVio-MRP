...
from kf import kf_update_moving_lm
...
class BiasedTree(object):
    ...
    """
    def __init__(self, workspace, geodesic, buchi, task, init_state, init_label, init_angle, segment, para, node_landmark = None):
        """
        ...
        if node_landmark:
            self.node_landmark = node_landmark 
        else:
    ...
    def biased_sample(self):
        ...
        for b_state in b_min_from_q_b_closest:
            candidate = []
            ''' Here I am replacing with direct shortest path candidate'''
            ...
            candidate.append(self.buchi.min_length_target[(b_state, self.b_final)])
            '''Replace end'''
            if candidate:
        ...
        transition_skill = self.buchi.buchi_graph.edges[(q_b_min, q_b_decr)]['transition_skill']
        ...
        return self.buchi_guided_sample_by_truthvalue(truth, AP_truth, avoid_targets, avoid_targets_2, x_rand, q_p_closest,
                                                      ...
                                                      x_angle, q_b_decr,(q_b_min, q_b_decr),
                                                      transition_skill)

    def buchi_guided_sample_by_truthvalue(self, truth, AP_truth, avoid_targets, 
                                          avoid_targets_2, x_rand, q_p_closest, x_label,
                                          x_angle, target_b_state, edge, transition_skill):
        """
            ...
            for key in truth:
                ...
                if truth[key] and found==False:
                    ...
                    while True:
                        ...
                        count+=1
                        # print("\n")
                        # print("avoid targets:",avoid_targets)
                        # print("avoid targets 2:",avoid_targets_2)
                        target, lm_pos, target_lm = self.get_target(orig_x_rand, pair[0], 
                        ...
                        distance_from_lm = self.dist_from_landmark(orig_x_rand, target_lm)
                        ...
                        if distance_from_lm>self.sensor_range:    
                            ...
                            if np.random.uniform(0, 1, 1) <= 0.9909:
                                x_rand[robot_index], x_angle[robot_index] = self.sample_control_to_target(orig_x_rand, orig_angle, target, lm_pos)
                                for lm_key in self.node_landmark.landmark.keys():
                                    if self.dist_from_landmark(x_rand[robot_index], lm_key) < self.sensor_range and self.update_covariance :
                                        self.node_landmark.landmark[lm_key][1] = ekf_update(self.node_landmark.landmark[lm_key][1]
        ...
        self.node_landmark.classes=self.workspace_instance.classes
    
        
        # self.node_landmark.landmark['l1'][0] = self.node_landmark.landmark['l1'][2].move_lm()
        # self.node_landmark.landmark['l1'][1] = kf_update_moving_lm(self.node_landmark.landmark['l1'][1], np.eye(2), np.array([[0.75,0],[0,0.76]]),self.node_landmark.landmark['l1'][2].vel)
        # self.node_landmark.generate_samples_for_lm('l1')
        
        # self.node_landmark.landmark['l2'][0] = self.node_landmark.landmark['l2'][2].move_lm()
        # self.node_landmark.landmark['l2'][1] = kf_update_moving_lm(self.node_landmark.landmark['l2'][1], np.eye(2), np.array([[0.26,0],[0,0.26]]),self.node_landmark.landmark['l2'][2].vel)        
        # self.node_landmark.generate_samples_for_lm('l2')
        
        # self.node_landmark.landmark['l3'][0] = self.node_landmark.landmark['l3'][2].move_lm()
        # self.node_landmark.landmark['l3'][1] = kf_update_moving_lm(self.node_landmark.landmark['l3'][1], np.eye(2), np.array([[0.16,0],[0,0.16]]),self.node_landmark.landmark['l3'][2].vel)        
        # self.node_landmark.generate_samples_for_lm('l3')
        
        # self.node_landmark.landmark['l4'][0] = self.node_landmark.landmark['l4'][2].move_lm()
        # self.node_landmark.landmark['l4'][1] = kf_update_moving_lm(self.node_landmark.landmark['l4'][1], np.eye(2), np.array([[0.16,0],[0,0.16]]),self.node_landmark.landmark['l4'][2].vel)        
        # self.node_landmark.generate_samples_for_lm('l4')
        
        # self.node_landmark.landmark['l5'][0] = self.node_landmark.landmark['l5'][2].move_lm()
        # self.node_landmark.landmark['l5'][1] = kf_update_moving_lm(self.node_landmark.landmark['l5'][1], np.eye(2), np.array([[0.16,0],[0,0.16]]),self.node_landmark.landmark['l5'][2].vel)        
        # self.node_landmark.generate_samples_for_lm('l5')
        
        # self.node_landmark.landmark['l6'][0] = self.node_landmark.landmark['l6'][2].move_lm()
        # self.node_landmark.landmark['l6'][1] = kf_update_moving_lm(self.node_landmark.landmark['l6'][1], np.eye(2), np.array([[0.16,0],[0,0.16]]),self.node_landmark.landmark['l6'][2].vel)        
        # self.node_landmark.generate_samples_for_lm('l6')
        
        # self.node_landmark.landmark['l7'][0] = self.node_landmark.landmark['l7'][2].move_lm()
        # self.node_landmark.landmark['l7'][1] = kf_update_moving_lm(self.node_landmark.landmark['l7'][1], np.eye(2), np.array([[0.16,0],[0,0.16]]),self.node_landmark.landmark['l7'][2].vel)        
        # self.node_landmark.generate_samples_for_lm('l7')
        
        # self.node_landmark.landmark['l8'][0] = self.node_landmark.landmark['l8'][2].move_lm()
        # self.node_landmark.landmark['l8'][1] = kf_update_moving_lm(self.node_landmark.landmark['l8'][1], np.eye(2), np.array([[0.16,0],[0,0.16]]),self.node_landmark.landmark['l8'][2].vel)        
        # self.node_landmark.generate_samples_for_lm('l8')
        
        # print(x_rand)
        # # print(self.node_landmark.landmark)        
        
        label = self.task.get_label_landmark(x_rand, self.node_landmark,
                                             self.buchi.buchi_graph.edges[edge]['task_label_ref'])
        # print("\n")
        # print(label)
        # print(truth)
        # print(transition_skill)
        self.package_handler(self.node_landmark, transition_skill, label, x_rand)
        
        return tuple(x_rand), x_angle, q_p_closest, label, target_b_state

    def package_handler(self, lm_obj, transition_skill, label, rob_pos):
        for drop_skill in lm_obj.package_drop_skills:
            for lm_key in transition_skill:
                if transition_skill[lm_key] == drop_skill:
                    pkg_drop = False
                    for AP in label:
                        if lm_key in label[AP]:
                            pkg_drop = True
                    if pkg_drop:
                        pair = lm_key.split('_')  # region-robot pair
                        robot_index = int(pair[1]) - 1
                        for pkg in lm_obj.packages_picked:
                            if pkg.picked_by_robot == robot_index and drop_skill in pkg.drop_skill:
                                pkg.landmark_id = pair[0]
                                pkg.position = lm_obj.landmark[pkg.landmark_id][0]
                                pkg.picked = False
                                pkg.picked_by_robot = None
                        lm_obj.update_package_locations()
                    
        for pick_skill in lm_obj.package_pick_skills:
            for lm_key in transition_skill:
                if transition_skill[lm_key] == pick_skill:
                    pkg_pick = False
                    for AP in label:
                        if lm_key in label[AP]:
                            pkg_pick = True
                    if pkg_pick:
                        pair = lm_key.split('_')  # region-robot pair
                        robot_index = int(pair[1]) - 1
                        for pkg in lm_obj.package_locations[pair[0]]:
                            # if robot_index not in lm_obj.packages_picked_robots and pick_skill in pkg.pick_skill :
                            if pick_skill in pkg.pick_skill :
                                pkg.landmark_id = None
                                # pkg.position = rob_pos[robot_index]
                                pkg.position = [200,200]
                                pkg.picked = True
                                pkg.picked_by_robot = robot_index
                                lm_obj.update_package_locations()
        for pkg in lm_obj.packages_picked:
            robot_index = pkg.picked_by_robot
            # pkg.position = rob_pos[robot_index]
            pkg.position = [200,200]
        lm_obj.update_package_locations()
    ...
    def get_target(self, init, target, avoid_targets, avoid_targets_2):
        ...
        tg_pos = self.node_landmark.landmark[target][0]
        # shortest = self.g.shortest_path(vg.Point(init[0], init[1]), vg.Point(tg_pos[0], tg_pos[1]))
        # start = datetime.datetime.now()
        avoid = avoid_targets.copy()
        for i in range(len(avoid_targets_2)):
            avoid.append(avoid_targets_2[i])
        waypoint = self.geodesic.get_geodesic_target2(init, tg_pos, self.node_landmark, avoid)
        # print(go_to)
        # NBA_time = (datetime.datetime.now() - start).total_seconds()
        # print('Time for getting path: {0:.4f} s'.format(NBA_time))
        return waypoint, tg_pos, target
    ...
    def sample_control_to_target(self, x, orig_angle, target, lm_pos):
        ...
        """discrete dynamics - High speed"""
        tau = 0.5
        if math.sqrt((lm_pos[1] - x[1])**2 + (lm_pos[0] - x[0])**2) < 10:
            dd = 6
        # elif math.sqrt((lm_pos[1] - x[1])**2 + (lm_pos[0] - x[0])**2) < 20:
            # dd = 12     # only for hardware experiment
        else:
            dd=18
            # dd = 12     # only for hardware experiment
        d = dd*tau
        ...
        for node in near_nodes:
            ...
            if q_new != node and obs_check[(q_new[0], node[0])] and \
                ...
                if c < cost:
                    ...
                    q_min = node
                    cost = c
        if added and not self.biased_tree.has_node(q_new) and self.buchi.buchi_graph.edges[(q_min[1],q_new[1])]['weight']<=100.5:
            # print(q_min[1],q_new[1],": ",self.buchi.buchi_graph.edges[(q_min[1],q_new[1])]['weight'])
            # print(q_new,label)
